<style>
    .description-td{
        max-width: 400px; /* Set a fixed width for the td */
        white-space: nowrap; /* Prevent text from wrapping */
        overflow: hidden; /* Hide overflowed text */
        text-overflow: ellipsis; /* Show ellipsis (...) for overflowed text */
        cursor: pointer; /* Change cursor to indicate the cell is interactive */
        transition: all 0.3s ease; /* Smooth transition for hover */
    }
    .description-td:hover {
        white-space: normal; /* Allow text to wrap */
        max-width: 500px; /* Optionally expand the width on hover */
        overflow: visible; /* Show the full text */
        word-wrap: break-word; /* Break long words if necessary */
        background-color: #f9f9f9; /* Optional: Change background on hover */
        box-shadow: 0px 0px 8px rgba(0, 0, 0, 0.1); /* Optional: Add shadow for better visibility */
    }
</style>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="card-actions">
                <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add-edit-expence"> Add new </button>
            </div>
        </div>
                ...
                @forelse ($expenses as $expence)
                    <tr>
                        ...
                        <td class="align-middle text-center">
                            ...
                            <x-button class="btn-icon btn btn-outline-info edit-btn" data-expence={{$expence}} > <x-icon.pencil/></x-button>
                            <x-button class="btn-icon  btn btn-outline-danger" id="del-btn" > <x-icon.trash/></x-button>
                        </td>
                    </tr>
                @empty
                    <tr>
...
@push('page-scripts')
    <script>
        $(document).ready(function () {
            var recordId = <?php echo json_encode($expence->id); ?>;
